...
mongo_server_uri =os.environ['mongo_server_uri']
...
def get_key_country(delt_name=None):
    out = db.delivery.find({'name': delt_name}, {'platform': 1, 'country': 1, 'roas_average_floor': 1, '_id': 0})
    out = list(out)
    if len(out) == 0:
        return None, None, None
    rand_index = choice(np.arange(len(out)), 1)[0]
    return out[rand_index]['platform'], out[rand_index]['country'], out[rand_index]['roas_average_floor']


def insert_baits(pts=None):
    db.baits_2018_09.insert_many(pts)
    return len(pts)
...
def test_ga_maker():
    out = db.delivery.distinct('name', {'class': {'$regex': '^S','$options': 'i'}})
    out = list(out)
    return out
