...
class Weight_Setting:
    def __init__(self):
        ...
        self.creative_attribute = dict()
        self.videoid_attribute = dict()
        self.urls_attribute = dict()
    ...
    def find_ads(self, report_ads=None):
        ...
        colles_ads = self.db.ads.find({'ad_id': {'$in': adids}}, {'_id': 0, 'ad_id': 1,
                                                                  ...
                                                                  'pt.creative.object_story_spec.video_data.video_id': 1
                                                                  })
        for ads in colles_ads:
                ...
                if pt['adset_spec']['targeting'].get('behaviors'):
                    ...
                    if isinstance(behaviors, list):
                        for behavior in behaviors:
                            self.is_add_attribute(behavior['id'], report_ads[ad_id], self.behaviors_attribute)
            ...
            if pt.get('creative') and pt['creative'].get('object_story_spec') and pt['creative']['object_story_spec'].get('video_data') \
                    and pt['creative']['object_story_spec']['video_data'].get('video_id'):
                videoid = pt['creative']['object_story_spec']['video_data']['video_id']
                self.is_add_attribute(videoid, report_ads[ad_id], self.videoid_attribute)
    ...
    def update_behavior(self):
        logger.info('update behavior')
        adid_weight = self.calc_weight(self.behaviors_attribute)
        rate = 0.1
        sql = 'SELECT pid,id,`name`,weight FROM dw_dim_behavior where id in %s' % str(tuple(adid_weight['id']))
        dm_behavior = pd.read_sql(sql, self.mysql_conn)
        sql = 'update dw_dim_behavior set weight=%f where pid=%d'
        adid_weight.columns = ['id', 'update_weight']
        dw_behavior = pd.merge(adid_weight, dm_behavior, on=['id'], how='inner')
        logger.info('the size of updating behavior:%d' % (len(dw_behavior)))
        for index in range(len(dw_behavior)):
            row = dw_behavior.iloc[index]
            tmp = row['update_weight'] + rate * (row['update_weight'] - float(row['weight']))
            if tmp > 1:
                tmp = 1
            if tmp < 0:
            ...
            self.mysql_cursor.execute(sql % (tmp, row['pid']))
            self.mysql_conn.commit()
            print(sql % (tmp, row['pid']))

    ### 更新coordinate的权重操作
    def update_creative_media(self):
        logger.info('update creative')
        adid_weight = self.calc_weight(self.videoid_attribute)
        rate = 0.1
        sql = 'SELECT pid,videoId as id,weight FROM dw_dim_creative_text where videoId in %s' % str(tuple(adid_weight['id']))
        dm_creative = pd.read_sql(sql, self.mysql_conn)
        sql = 'update dw_dim_creative_text set weight=%f where pid=%d'
        adid_weight.columns = ['id', 'update_weight']
        dw_creative = pd.merge(adid_weight, dm_creative, on=['id'], how='inner')
        logger.info('the size of updating dw_dim_creative_text:%d' % (len(dw_creative)))
        for index in range(len(dw_creative)):
            row = dw_creative.iloc[index]
            tmp = row['update_weight'] + rate * (row['update_weight'] - float(row['weight']))
            if tmp > 1:
                tmp = 1
            if tmp < 0:
